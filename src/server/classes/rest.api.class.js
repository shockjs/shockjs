...
import AuthManager from './auth.manager.class';
...
class RestApiClass
{
  ...
  constructor(endPoint, model, version='v1', perPageLimit=50, perPage=20)
  {
     ...
     * Finds all instances.
     */
    this.findAll = this.beforeAction((request, reply) => {
     ...
     */
    this.fetchCount = this.beforeAction((request, reply) => {
      this.model
        .query((qb) => {
     ...
     */
    this.fetchColumns = this.beforeAction((request, reply) => {
     ...
     */
    this.fetchDefaults = this.beforeAction((request, reply) => {
      let model = new this.model();
      let tableName = model.tableName;
      if (this.model.knex !== undefined) {
        this.model.knex().raw(`SHOW COLUMNS FROM ${tableName}`)
     ...
     */
    this.findOne = this.beforeAction((request, reply) => {
     ...
     */
    this.createOne = this.beforeAction((request, reply) => {
     ...
     */
    this.updateOne = this.beforeAction((request, reply) => {
      let payload = request.payload;
     ...
     */
    this.deleteOne = this.beforeAction((request, reply) => {
  ...
  beforeAction(handler)
  {
    return {
      handler: (request, reply) => {
        this.checkPermissions(request, reply)
          .then(() => {
            handler(request, reply);
          })
          .catch(() => {
  ...
  checkPermissions(request, reply)
  {
    return new Promise((resolve, reject) => {
      if (this.accessRules) {
        let rules = this.accessRules();
        console.log(rules);
        rules.forEach((rule) => {
          resolve(true); // @todo do checks if permissions are set here.
        });
        if (rules.length === 0) {
          resolve(true);
        }
    ...
    return new Promise((resolve, reject) => {
      if (!request.auth.isAuthenticated) {
        ...
        if (this._userLookup !== null) {
          resolve(this._userLookup);
        } else {
          var model = this.model.forge({});
          model.query({where: { id: request.auth.credentials.id }})
            .fetch()
            .then((modelInstance) => {
              this._userLookup = modelInstance;
